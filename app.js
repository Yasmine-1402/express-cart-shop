...
// AJV v8: disable strict mode for older schemas and add common formats
...
// Support both older and newer express-handlebars APIs
app.engine('hbs', (handlebars.engine || handlebars)({
    extname: 'hbs',
...
handlebars = handlebars.create({
    helpers: {
        // Language helper
        __: function() { return i18n.__(this, arguments); }, // eslint-disable-line no-undef
        __n: function() { return i18n.__n(this, arguments); }, // eslint-disable-line no-undef
...
app.use(session({
    ...
    cookie: {
        path: '/',
        httpOnly: true,
        maxAge: parseInt(process.env.SESSION_MAX_AGE || '900000'),
...
app.use('/customer/login_action', authLimiter);
app.use('/admin/login_action', authLimiter);
app.use('/customer/create', authLimiter);

// Optional CSRF protection (disabled by default). Enable by setting ENABLE_CSRF=true in .env
if(process.env.ENABLE_CSRF === 'true'){
    const csrfProtection = csurf();
    // Skip CSRF for known webhook endpoints to avoid breaking external callbacks
    const csrfSkip = [
    ...
    app.use((req, res, next) => {
        if(csrfSkip.some(p => req.path.startsWith(p))){
            return next();
        }
...
initDb(config.databaseConnectionString, async (err, db) => {
    ...
    try{
        ...
        // Graceful shutdown
        const shutdown = () => {
            console.log(colors.yellow('Received kill signal, shutting down gracefully.'));
            server.close(() => {
                console.log(colors.yellow('Closed out remaining connections.'));
                if(app.db && app.db.client) {
                    app.db.client.close().then(() => {
                        console.log(colors.yellow('Closed MongoDB connection.'));
                        process.exit(0);
                    }).catch((err) => {
                        console.error(colors.red(`Error closing MongoDB: ${err.message}`));
                        process.exit(1);
                    });
                } else {
                    process.exit(0);
            ...
            setTimeout(() => {
                console.error(colors.red('Could not close connections in time, forcefully shutting down.'));
                process.exit(1);
            }, 10000);
        };
