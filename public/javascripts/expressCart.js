...
$(document).ready(function (){
    // ---- Dynamic Country → City toggling ----
    function toggleCityField(countrySelect) {
        var $select = $(countrySelect);
        var citySelectId = $select.data('city-select');
        var cityInputId = $select.data('city-input');
        if (!citySelectId || !cityInputId) return;
        ...
        if (selectedCountry === 'Egypt') {
            $citySelect.show().prop('disabled', false).attr('name', 'shipState');
            $cityInput.hide().prop('disabled', true).removeAttr('name');
        } else {
            $citySelect.hide().prop('disabled', true).removeAttr('name');
            $cityInput.show().prop('disabled', false).attr('name', 'shipState');
            $cityInput.attr('placeholder', 'Enter city in ' + selectedCountry);
        }
    }

    // Run on page load for all country selectors
    $('.country-selector').each(function() {
        toggleCityField(this);
    });
    ...
    $('#customer-signup-form').on('submit', function(e){
        ...
        var phone = $('#signup-phone').val() || 'N/A';
        var address1 = $('#signup-address1').val() || 'N/A';
        var city = ($('#signup-city').is(':visible') ? $('#signup-city').val() : $('#signup-cityText').val()) || 'N/A';
        var postcode = 'N/A';
        var country = $('#signup-country').val() || 'Egypt';
        ...
        if(password.length < 5){
            showNotification('Password must be at least 5 characters', 'danger');
            return;
        }
        ...
        .done(function(msg){
            showNotification('Account created successfully!', 'success', false, '/customer/account');
        })
        .fail(function(msg){
            if(msg.responseJSON){
                if(msg.responseJSON.message){
                    showNotification(msg.responseJSON.message, 'danger');
                }else if(Array.isArray(msg.responseJSON)){
                    var firstError = msg.responseJSON[0];
                    showNotification(firstError.message || 'Validation error. Please check your fields.', 'danger');
                }else{
                    showNotification('Account creation failed. Please try again.', 'danger');
                }
            }else{
                showNotification('Account creation failed. Please try again.', 'danger');
            }
    ...
    $('#loginForm').closest('form').on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
            $.ajax({
                method: 'POST',
    ...
    $('#customerloginForm').closest('form').on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
            $.ajax({
                method: 'POST',
    ...
    $('#customerLogin').closest('form').on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
            $.ajax({
                method: 'POST',
