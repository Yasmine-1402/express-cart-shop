...
$(document).ready(function (){
    ...
    $('#checkoutCod').validator().on('click', function(e){
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: '/cod/checkout_action'
        }).done(response => {
            window.location = '/payment/' + response.paymentId;
        }).fail(response => {
            window.location = '/payment/' + response.paymentId;
    ...
    $('#checkoutInstapay').validator().on('click', function(e){
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: '/instapay/checkout_action'
        }).done(response => {
            window.location = '/payment/' + response.paymentId;
        }).fail(response => {
            window.location = '/payment/' + response.paymentId;
    ...
    $('#checkoutPaymob').validator().on('click', function(e){
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: '/paymob/checkout_action'
        }).done(response => {
            window.location = response.redirectUrl;
        }).fail(response => {
            showNotification(response.responseJSON ? response.responseJSON.message : 'Failed to connect to payment gateway', 'danger');
        });
    });
