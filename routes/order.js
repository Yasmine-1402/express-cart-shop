...
router.get('/admin/orders/bystatus/:orderstatus', restrict, async (req, res, next) => {
    ...
    const regex = new RegExp(['^', escapeRegex(req.params.orderstatus), '$'].join(''), 'i');
...
router.post('/admin/order/create', restrict, async (req, res, next) => {
    const db = req.app.db;
...
// order product
router.post('/admin/order/delete', restrict, async(req, res) => {
    const db = req.app.db;

    // remove the order
    try{
        await db.orders.deleteOne({ _id: getId(req.body.orderId) });
