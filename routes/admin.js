...
router.post('/admin/settings/page', restrict, checkAccess, async (req, res) => {
    ...
    if(req.body.pageId){
        ...
        try{
            const updatedPage = await db.pages.findOneAndUpdate({ _id: getId(req.body.pageId) }, { $set: doc }, { returnDocument: 'after' });
...
router.post('/admin/validatePermalink', restrict, async (req, res) => {
...
router.post('/admin/searchall', restrict, async (req, res, next) => {
    ...
    if(emailRegex.test(req.body.searchValue)){
        ...
        customerQuery.$or = [
            { firstName: { $regex: new RegExp(escapedSearchValue, 'img') } },
            { lastName: { $regex: new RegExp(escapedSearchValue, 'img') } }
        ];
        orderQuery.$or = [
            { orderFirstname: { $regex: new RegExp(escapedSearchValue, 'img') } },
            { orderLastname: { $regex: new RegExp(escapedSearchValue, 'img') } }
        ];
        productQuery.$or = [
            { productTitle: { $regex: new RegExp(escapedSearchValue, 'img') } },
            { productDescription: { $regex: new RegExp(escapedSearchValue, 'img') } }
        ];
    }
