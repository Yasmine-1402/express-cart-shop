...
router.get('/health', async (req, res) => {
    try {
        await req.app.db.admin().ping();
        res.status(200).json({ status: 'UP', database: 'UP' });
    } catch (err) {
        res.status(503).json({ status: 'DOWN', database: 'DOWN', error: err.message });
    }
});
...
router.get('/payment/:orderId', async (req, res, next) => {
    ...
    if(config.trackStock && req.session.paymentApproved){
        ...
        if(order.productStockUpdated !== true && order.orderProducts){
...
router.get('/checkout/cartdata', (req, res) => {
    ...
    res.status(200).json({
        ...
        session: {
            totalCartAmount: req.session.totalCartAmount,
            totalCartShipping: req.session.totalCartShipping,
            shippingMessage: req.session.shippingMessage,
            totalCartDiscount: req.session.totalCartDiscount,
...
router.post('/product/addtocart', async (req, res, next) => {
    ...
    if(req.session.cart[productCartId]){
        ...
        // new product deets
        const productObj = {};
        productObj.productId = product._id.toString();
        ...
        if(product.productPermalink){
            ...
            productObj.link = product._id.toString();
        }
...
router.post('/contact/send', async (req, res) => {
    ...
    try {
        ...
        const contactMessage = {
            ...
            phone: phone ? stripHtml(phone).result : '',
            subject: stripHtml(subject).result,
        ...
        await db.contacts.insertOne(contactMessage);

        // Send confirmation email to customer
        const customerEmailBody = `
            <h2>Thank You for Contacting ${config.cartTitle}</h2>
            <p>Dear ${stripHtml(name).result},</p>
            <p>We have received your message and will get back to you as soon as possible.</p>
            <hr/>
            <h3>Your Message Details:</h3>
            <p><strong>Subject:</strong> ${stripHtml(subject).result}</p>
            <p><strong>Message:</strong> ${stripHtml(message).result.replace(/\n/g, '<br>')}</p>
            <hr/>
            <p>Best regards,<br/>${config.cartTitle} Team</p>
        `;

        sendEmail(email, `Re: ${stripHtml(subject).result} - ${config.cartTitle}`, customerEmailBody);

        // Send notification email to store owner
        const ownerEmailBody = `
            <h2>New Contact Form Submission</h2>
            <p><strong>From:</strong> ${stripHtml(name).result}</p>
            <p><strong>Email:</strong> ${email}</p>
            <p><strong>Phone:</strong> ${phone ? stripHtml(phone).result : 'Not provided'}</p>
            <p><strong>Subject:</strong> ${stripHtml(subject).result}</p>
            <hr/>
            <h3>Message:</h3>
            <p>${stripHtml(message).result.replace(/\n/g, '<br>')}</p>
            <hr/>
            <p><a href="http://localhost:1111/admin/settings/pages">View in Admin Panel</a></p>
        `;

        sendEmail(config.emailAddress, `New Contact: ${stripHtml(subject).result}`, ownerEmailBody);
