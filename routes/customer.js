...
router.post('/customer/create', async (req, res) => {
    ...
    try{
        ...
        .then(() => {
            // Return the new customer with inserted ID
            ...
            req.session.customerId = customerReturn._id.toString();
            ...
            // Return customer object (without password)
...
router.post('/customer/save', async (req, res) => {
    ...
    // Set the customer into the session
    req.session.customerPresent = true;
    req.session.customerEmail = sanitizeHtml(customerObj.email, sanitizeConfig);
    req.session.customerCompany = sanitizeHtml(customerObj.company, sanitizeConfig);
    req.session.customerFirstname = sanitizeHtml(customerObj.firstName, sanitizeConfig);
    req.session.customerLastname = sanitizeHtml(customerObj.lastName, sanitizeConfig);
    req.session.customerAddress1 = sanitizeHtml(customerObj.address1, sanitizeConfig);
    req.session.customerAddress2 = sanitizeHtml(customerObj.address2, sanitizeConfig);
    req.session.customerCountry = sanitizeHtml(customerObj.country, sanitizeConfig);
    req.session.customerState = sanitizeHtml(customerObj.state, sanitizeConfig);
    req.session.customerPostcode = sanitizeHtml(customerObj.postcode, sanitizeConfig);
    req.session.customerPhone = sanitizeHtml(customerObj.phone, sanitizeConfig);
    req.session.orderComment = req.body.orderComment ? sanitizeHtml(req.body.orderComment, sanitizeConfig) : '';
...
router.get('/customer/signup', async (req, res, next) => {
    const config = req.app.config;
