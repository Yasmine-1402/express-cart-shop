...
const getConfig = () => {
    ...
    // Common environment variable fallbacks
    if(process.env.MONGO_URI && (!config.databaseConnectionString || config.databaseConnectionString === '')){
        config.databaseConnectionString = process.env.MONGO_URI;
    }
    if(process.env.DATABASE_URL && (!config.databaseConnectionString || config.databaseConnectionString === '')){
        config.databaseConnectionString = process.env.DATABASE_URL;
    }
    if(process.env.EMAIL_USER && (!config.emailUser || config.emailUser === '')){
        config.emailUser = process.env.EMAIL_USER;
    }
    if(process.env.EMAIL_PASSWORD && (!config.emailPassword || config.emailPassword === '')){
        config.emailPassword = process.env.EMAIL_PASSWORD;
    }
    if(process.env.SECRET_COOKIE && (!config.secretCookie || config.secretCookie === '')){
        config.secretCookie = process.env.SECRET_COOKIE;
    }
    if(process.env.SECRET_SESSION && (!config.secretSession || config.secretSession === '')){
        config.secretSession = process.env.SECRET_SESSION;
    }
