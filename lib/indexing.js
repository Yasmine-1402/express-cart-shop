...
const indexProducts = async (app) => {
    ...
    const productsIndex = lunr(function(){
        ...
        this.field('productType', { boost: 5 });
        this.field('ageRange');
        this.field('learningLevel');
        ...
        for(const product of productsList){
            const doc = {
                ...
                learningLevel: product.learningLevel,
...
const indexCourses = async (app) => {
    if(!app.db.courses) return;
    const coursesList = await app.db.courses.find({}).toArray();

    const coursesIndex = lunr(function(){
        this.field('courseTitle', { boost: 10 });
        this.field('instructor', { boost: 5 });
    ...
    app.coursesIndex = coursesIndex;
    if(process.env.NODE_ENV !== 'test'){
        console.log(colors.cyan('- Course indexing complete'));
    }
};
