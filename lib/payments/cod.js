...
router.post('/checkout_action', async (req, res, next) => {
    ...
    const paymentConfig = getPaymentConfig('cod');

    const orderDoc = {
        orderPaymentId: getId(),
        orderPaymentGateway: 'Cash on Delivery',
        orderPaymentMessage: 'Your order is pending. An admin will call you to confirm your order. Cash on delivery is required.',
    ...
    try{
        ...
        .then(async () => {
            ...
            req.session.message = 'Your order is pending. An admin will call you to confirm your order. Cash on delivery is required.';
            ...
            try {
                await sendEmail(req.session.paymentEmailAddr, `Your order with ${config.cartTitle}`, getEmailTemplate(paymentResults));
            } catch (emailErr) {
        ...
        console.error('COD checkout error:', ex);
